...
def diophantine1_enumeration(
    ...
    try:
        ...
        combined_pattern = re.compile(
            rf"^(?:({double_regex})|({single_regex}))$"
        )
        
        subtype_match = combined_pattern.fullmatch(all_facts)
        if not subtype_match: return False
        
        if subtype_match.group(1):
            
            facts_match = double_pattern.fullmatch(all_facts)
            assert facts_match
        ...
        elif subtype_match.group(2):
            
            facts_match = single_pattern.fullmatch(all_facts)
            assert facts_match
            
            x, x2, y, y2, a, x3, b, c, y3, x_or_y, max_value = \
                facts_match.group(1), facts_match.group(2), facts_match.group(3), \
                facts_match.group(4), int(facts_match.group(5)), facts_match.group(6), \
                int(facts_match.group(7)), int(facts_match.group(8)), facts_match.group(9), \
            ...
            if x_or_y != x and x_or_y != y: return False
            
            true_solutions = _exhaust_diophantine1(a, b, c, max_value, None) \
                if x_or_y == x else _exhaust_diophantine1(a, b, c, None, max_value)
        ...
        if _verbose:
            print(
                f"diophantine1_enumeration: prop {prop} has been revalidated "
                f"by the following verified facts:\n{lined_facts}"
            )
...
diophantine_rechecker = CoLeanRechecker(
    claim_keyword = "Claim",
    prop_field = "prop",
)


diophantine_rechecker.add_revalidators(
    revalidators = [
        ("pow_mod_eq_zero", \
            pow_mod_eq_zero),
        ("observe_mod_cycle", \
